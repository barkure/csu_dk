...
def test_captcha_is_consumed_atomically():
    """并发提交同一个验证码：只能有一个成功，否则就是一个认证边界漏洞。"""
...
def test_relogin_cooldown_lives_in_service_layer():
    ...
    # 第一次：允许进入（随后因为测试里禁止真实登录而失败，但不再是限流）
...
def test_schema_has_what_the_code_needs():
    """库里现有结构必须覆盖代码依赖的表与列，records 索引还得带 account_id。"""
    ...
    for column in ("auth_error", "needs_reauth", "password_enc", "cookies", "token_at"):
...
def test_lease_stale_follows_scan_interval(monkeypatch):
    """扫描间隔能配到 3600 秒，租约失效时间必须跟着走。

    否则正常持有者还没轮到下一次心跳就被别人判成过期，两个进程会轮流"接管"。
    """
    from app import scheduler as sched
...
def test_bootstrap_runs_only_for_the_lease_holder(monkeypatch):
    """排期修正只在真的持有租约后做一次：多进程启动时不能每个进程都去改调度状态。"""
    from app import scheduler as sched

    called = []
    monkeypatch.setattr(sched, "bootstrap", lambda: called.append(1))
    monkeypatch.setattr(sched, "_bootstrapped", False)
    monkeypatch.setattr(sched, "_lease_owner", "test:ours")
    now = local_now(cfg.config.tz)

    # 租约被别人占着 → 既不接管也不 bootstrap
    assert db.acquire_scheduler_lease("test:other", to_local_iso(now), to_local_iso(now)) is True
    assert sched._holds_lease(now) is False
    assert called == []

    # 让出租约 → 接管并只 bootstrap 一次
    db.release_scheduler_lease("test:other")
    assert sched._holds_lease(now) is True
    assert sched._holds_lease(now) is True
    assert called == [1]

    db.release_scheduler_lease("test:ours")


def test_maintenance_respects_lease(monkeypatch):
    """维护任务会登录学校补楼栋名，多进程时必须只有持租约的那个进程做。"""
    from app import scheduler as sched

    calls = []
    monkeypatch.setattr(sched, "fill_missing_addresses", lambda now: calls.append(now))
    monkeypatch.setattr(sched, "_lease_owner", "test:ours")
    now = local_now(cfg.config.tz)

    db.acquire_scheduler_lease("test:other", to_local_iso(now), to_local_iso(now))
    sched.maintenance()
    assert calls == [], "非租约持有者不该去访问学校"
...
def test_structure_mismatch_is_reported():
    """结构不符时要明确报错并说清缺什么，而不是等运行时才炸 no such column。"""
...
def test_structure_is_checked_before_ddl_runs():
    """已有库缺列时，要在建索引之前就报出明确提示。

    否则 CREATE INDEX ... ON accounts(auth_error) 会先抛 "no such column: auth_error"，
    用户看到的是 SQL 报错，而不是"请按最新结构重建数据库"。
    """
    import re
    import sqlite3
    import tempfile
    ...
    # 先把"缺 auth_error 的旧结构"建出来（索引一并去掉，否则这一步就建不起来）
